...
st.markdown("""
    ...
    /* --- NEW TCC POPUP/CARD STYLES (Matches Figma 227) --- */
    .tcc-card {
        background: white;
        border-radius: 16px;
        padding: 35px;
    ...
    .tcc-card-title { color: #1A56DB; font-size: 1.6rem; font-weight: 700; margin-bottom: 5px; }
    .tcc-card-subtitle { color: #6B7280; font-size: 0.95rem; margin-bottom: 25px; }
    ...
    .dr-img-container { width: 100%; height: 100%; }
    .dr-img { width: 100%; height: 280px; object-fit: cover; border-radius: 12px; box-shadow: 0 4px 6px rgba(0,0,0,0.05); }
    /* --------------------------------------------------- */

    .white-btn div.stButton > button { background: white !important; color: #4B5563 !important; border-radius: 8px; height: 50px; font-weight: 600; border: 1px solid #E5E7EB !important; margin-top: 10px; }
    </style>
...
with tab1:
    ...
    try: base_usage = int(usage)
    except: base_usage = 10
    for week in range(1, 13):
        target = max(0, base_usage - (week - 1)) 
        with st.expander(f"Week {week} - Target: {target} units/day", expanded=(week==1)):
            if week == 1: st.info("Current Week: Focus on strictly monitoring your intake.")
...
# --- NEW TCC SECTION EXACTLY LIKE FIGMA ---
st.markdown("""
    <div class="tcc-card">
        <div class="tcc-card-title">TCC - Tobacco Cessation Cell</div>
        <div class="tcc-card-subtitle">Get professional support for your recovery journey</div>
""", unsafe_allow_html=True)
...
with col_img:
    # Tumhara exact raw link yahan daal diya hai maine
